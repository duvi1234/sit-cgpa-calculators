...
// Honors subjects data
const honorsSubjects = {
    "Honors 1": { "name": "Honors 1", "credit": 3, "code": "HON001" },
    "Honors 2": { "name": "Honors 2", "credit": 3, "code": "HON002" }
};
...
function populateSemesters() {
    ...
    const honorsSection = document.getElementById("gpaHonorsSection");
    ...
    if (selectedBatch && departmentData["CSE (AI & ML)"][selectedBatch]) {
        ...
        // Show honors section if batch has semesters 5, 6, or 7
        const hasHonorsSemesters = semesters.some(sem => ["Semester V", "Semester VI", "Semester VII"].includes(sem));
        honorsSection.style.display = hasHonorsSemesters ? "block" : "none";
    } else {
        honorsSection.style.display = "none";
    }
}

// Function to check if a semester should show honors option
function shouldShowHonorsForSemester(semesterKey) {
    return ["Semester V", "Semester VI", "Semester VII"].includes(semesterKey);
}

// Function to add honors subjects to a semester block
function addHonorsSubjectsToSemester(semesterBlock, semesterKey, honorsEnabled) {
    if (!shouldShowHonorsForSemester(semesterKey) || !honorsEnabled) {
        return;
    }
    
    // Add honors subjects
    Object.values(honorsSubjects).forEach((honorsSubject, index) => {
        const row = document.createElement("div");
        row.classList.add("subject-row", "honors-subject-row");
        
        const code = document.createElement("div");
        code.textContent = honorsSubject.code;
        code.classList.add("subject-cell");
        
        const name = document.createElement("div");
        name.textContent = honorsSubject.name;
        name.classList.add("subject-cell");
        
        const credit = document.createElement("div");
        credit.textContent = honorsSubject.credit;
        ...
        gradeSelect.name = `honors-grade-${semesterKey}-${index}`;
        gradeSelect.classList.add("grade-dropdown", "honors-grade-dropdown");
...
function renderCGPASubjects(batch, semesterCount) {
    ...
    const honorsEnabled = document.getElementById("cgpaHonorsCheckbox")?.checked || false;
    ...
    for (let i = 0; i < semesterCount; i++) {
        ...
        // Add honors subjects if enabled
        addHonorsSubjectsToSemester(semesterBlock, semesterKey, honorsEnabled);
...
function populateSubjects() {
    ...
    const semesterSelect = document.getElementById("semesterSelect");
    const subjectsContainer = document.getElementById("subjectsContainer");
    const honorsCheckbox = document.getElementById("gpaHonorsCheckbox");
    ...
    if (selectedBatch && selectedSemester && departmentData["CSE (AI & ML)"][selectedBatch][selectedSemester]) {
        ...
        // Add honors subjects if checkbox is checked and semester is 5, 6, or 7
        if (honorsCheckbox && honorsCheckbox.checked && shouldShowHonorsForSemester(selectedSemester)) {
            Object.values(honorsSubjects).forEach(honorsSubject => {
                addSubjectRow(honorsSubject.name, honorsSubject.credit, honorsSubject.code);
            });
        }
    }
}
...
function calculateCGPA() {
    ...
    // Calculate from semester subjects (including honors subjects)
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (cgpaBatchSelect) {
        cgpaBatchSelect.addEventListener('change', function() {
            ...
            const honorsSection = document.getElementById("cgpaHonorsSection");
            ...
            if (batch) {
                ...
                // Show honors section if batch has semesters 5, 6, or 7
                if (departmentData["CSE (AI & ML)"][batch]) {
                    const semesters = Object.keys(departmentData["CSE (AI & ML)"][batch]);
                    const hasHonorsSemesters = semesters.some(sem => ["Semester V", "Semester VI", "Semester VII"].includes(sem));
                    honorsSection.style.display = hasHonorsSemesters ? "block" : "none";
                } else {
                    honorsSection.style.display = "none";
    ...
    // Add honors checkbox event listeners
    const gpaHonorsCheckbox = document.getElementById("gpaHonorsCheckbox");
    const cgpaHonorsCheckbox = document.getElementById("cgpaHonorsCheckbox");
    
    if (gpaHonorsCheckbox) {
        gpaHonorsCheckbox.addEventListener('change', function() {
            // Re-populate subjects when honors checkbox changes
            const semesterSelect = document.getElementById("semesterSelect");
    ...
    if (cgpaHonorsCheckbox) {
        cgpaHonorsCheckbox.addEventListener('change', function() {
            // Re-render CGPA subjects when honors checkbox changes
            const batch = document.getElementById("cgpaBatchSelect").value;
            const semesterCount = document.getElementById("semesterCountSelect").value;
            if (batch && semesterCount) {
                renderCGPASubjects(batch, parseInt(semesterCount));
            }
        });
    }
});
